...
std::tuple<cv::Mat, cv::Mat, cv::Size>
read_calibration(const std::string &filename) {
...
int main(void) {
    ...
    auto [camera_matrix, dist_coeffs, image_size] =
        read_calibration("calibration.yml");
    ...
    double marker_length = 0.145; // Length of the marker in meters
    ...
    while (true) {
        ...
        detector.detectMarkers(frame_src, marker_corners, marker_ids,
                               rejected_candidates);

        if (marker_ids.size() > 0) {
            cv::aruco::drawDetectedMarkers(frame_output, marker_corners,
                                           marker_ids);
            cv::imshow("Detected Markers", frame_output);
            ...
            for (size_t i = 0; i < marker_ids.size(); ++i) {
                cv::drawFrameAxes(frame_output, camera_matrix, dist_coeffs,
                                  rvecs[i], tvecs[i], 0.1);
            }
        }
